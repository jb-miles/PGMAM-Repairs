#!/usr/bin/env node
/*
  Windows Harness: Plex -> Unique Actors -> IAFD URLs -> IAFD Photos -> PGMA folders

  Goals:
  - Interactive prompts (Plex URL, token, Plex support dir, _PGMA dir)
  - Install dependencies (Playwright + Chromium) if needed
  - Resumable (Plex export uses state; IAFD lookup uses --resume CSV)
  ...
  - Designed to work on Windows, but also works on macOS/Linux if paths are provided.
*/
...
function npmCmd() {
  // On Windows with spawn(..., {shell:false}), you typically need npm.cmd/npx.cmd.
...
function defaultWindowsPlexSupportCandidates() {
  const local = process.env.LOCALAPPDATA ? path.join(process.env.LOCALAPPDATA, 'Plex Media Server') : '';
  const roaming = process.env.APPDATA ? path.join(process.env.APPDATA, 'Plex Media Server') : '';
  const out = [];
  if (local) out.push(local);
  if (roaming && roaming !== local) out.push(roaming);
  return out;
}
...
function question(rl, prompt, { defaultValue = '', secret = false } = {}) {
  ...
  // Basic "secret" input: mask with * and avoid echoing actual characters.
  // This is best-effort; some terminals may still show characters.
...
async function maybeInstallPlaywright() {
  ...
  await runCmd(npmCmd(), ['install'], { cwd: IAFD_LOOKUP_DIR });
}

async function maybeInstallChromium() {
  // We don't try to be too clever. If the user already installed, this is fast.
  log('Ensuring Playwright Chromium is installed (npx playwright install chromium) ...');
  await runCmd(npxCmd(), ['playwright', 'install', 'chromium'], { cwd: IAFD_LOOKUP_DIR });
}
...
async function resolvePgmaPath(rl, plexSupportPath) {
  ...
  if (expected) {
    log(`_PGMA not found at expected path: ${expected}`);
  }
...
async function resolvePlexSupportPath(rl) {
  if (!isWin()) {
    // Non-Windows: just prompt.
    const ans = await question(rl, 'Enter Plex Media Server support directory', { defaultValue: '' });
    return ans.trim();
  }

  const candidates = defaultWindowsPlexSupportCandidates();
  for (const c of candidates) {
    if (c && (await pathExists(c))) {
      log(`Detected Plex support directory: ${c}`);
      return c;
    }
  }

  log('Could not auto-detect Plex support directory.');
  const hint = candidates.filter(Boolean)[0] || '';
  while (true) {
    const ans = await question(rl, 'Enter Plex Media Server support directory', { defaultValue: hint });
    const p = ans.trim();
    if (!p) continue;
...
async function chooseMovieSectionId(rl, plexBaseUrl, plexToken) {
  ...
  while (true) {
    ...
    const ok = movieSections.some((s) => s.key === chosen);
    if (ok) return chosen;
...
async function actorsCsvToList(actorsCsvPath) {
  ...
  for (let i = 1; i < lines.length; i += 1) {
    // actors.csv is simple: one column 'actor' (quoted sometimes)
    let s = lines[i];
    if (!s) continue;
...
async function main() {
  ...
  if (major < 20) {
    throw new Error(`Node.js v20+ required. You are running ${process.version}`);
  }

  if (!(await pathExists(PLEX_EXPORT_DIR)) || !(await pathExists(IAFD_LOOKUP_DIR))) {
    throw new Error(
      `Expected folders not found. Need:\n- ${PLEX_EXPORT_DIR}\n- ${IAFD_LOOKUP_DIR}\nRun this harness from the repo root.`
    );
  }

  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  try {
    log('Plex -> IAFD workflow harness starting.');
    ...
    const photoKind = (await question(rl, 'Download which photos? (poster|face|both)', { defaultValue: 'both' }))
      .trim()
      .toLowerCase();
    if (!['poster', 'face', 'both'].includes(photoKind)) {
      throw new Error('Invalid choice for photo kind. Use: poster, face, or both.');
    }
