#!/usr/bin/env node
/*
  Linux Harness: Plex -> Unique Actors -> IAFD URLs -> IAFD Photos -> PGMA folders
*/
...
function question(rl, prompt, { defaultValue = '', secret = false } = {}) {
  ...
  if (!secret) return new Promise((resolve) => rl.question(fullPrompt, (ans) => resolve(ans || defaultValue)));
...
function runCmd(cmd, args, { cwd, redact = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      reject(new Error(`Command failed (${code}): ${shown}`));
    });
  });
}
...
async function resolvePlexSupportPath(rl) {
  const candidates = [
    process.env.PLEX_MEDIA_SERVER_APPLICATION_SUPPORT_DIR || '',
    path.join(os.homedir(), '.config', 'Plex Media Server'),
    path.join(os.homedir(), '.local', 'share', 'Plex Media Server'),
...
async function chooseMovieSectionId(rl, plexBaseUrl, plexToken) {
  ...
  const movieSections = dirs
    ...
    .map((d) => ({ key: Number(d.key), title: String(d.title || '') }))
...
async function main() {
  ...
  try {
    log('Plex -> IAFD workflow harness starting (Linux).');
